#!/usr/bin/env node
//exemple d'appel: node list-tree-dir-cine C:\Users\frede\Music\ c.html
...
function buildTable(items, rootDir) {
  let html = '<table>\n  <colgroup>\n';
  ...
  for (let i = 0; i < items.length; i += 3) {
    html += '  <tr>';
    for (let j = 0; j < 3; j += 1) {
      const item = items[i + j] || null;
      const label = item ? item.name : '';
      const targetPath = item && rootDir ? path.join(rootDir, item.path) : '';
      const href = targetPath
        ? `/tools/open-file-local?target=${encodeURIComponent(targetPath)}`
        : '#';
      html += `<td><a href="${escapeHtml(href)}" target="_blank" rel="noopener noreferrer">${escapeHtml(label)}</a></td>`;
    }
    html += '</tr>\n';
  }
  html += '</tbody>\n</table>\n';
...
function buildHtml(tree, title, rootDir) {
  ...
  for (const level1 of tree) {
    ...
    if (level1.children.length > 0) {
      const rowValues = level1.children.map((child) => ({ name: child.name, path: child.path }));
      html += buildTable(rowValues, rootDir);
    }
  }
...
async function main() {
  ...
  if (ext === '.html' || ext === '.htm') {
    ...
    const html = buildHtml(tree, title, rootDir);
